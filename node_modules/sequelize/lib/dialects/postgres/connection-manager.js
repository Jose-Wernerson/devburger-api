...
class ConnectionManager extends AbstractConnectionManager {
	...
	_refreshTypeParser(dataType) {
		const arrayParserBuilder = (parser2) => {
			return (value) =>
				this.lib.types.arrayParser.create(value, parser2).parse()
		}
	...
	async connect(config) {
		...
		const connection = await new Promise((resolve, reject) => {
			...
			connection2.connect((err) => {
				responded = true
				if (!this.sequelize.config.native) {
					connection2.connection.removeListener(
						'parameterStatus',
						parameterHandler,
					)
				}
		...
		if (this.sequelize.options.clientMinMessages !== void 0) {
			console.warn(
				'Usage of "options.clientMinMessages" is deprecated and will be removed in v7.',
			)
			console.warn(
				'Please use the sequelize option "dialectOptions.clientMinMessages" instead.',
			)
		}
